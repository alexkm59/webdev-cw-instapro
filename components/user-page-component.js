...
export function renderUserPageComponent({ appEl, posts, page }) {
              ...
              <li class="post">
                    ...
                    <div class="post-image-container">
                     ...
                     <div class="post-likes">
                       <button data-post-id="${posts.id}" data-isliked="${(posts.isLiked) ? true : false}" data-user-id="${posts.user.id}" class="like-button like-button-user">
    ...
    const likeElementFunction = () => {
        const likeElements = document.querySelectorAll(".like-button-user");
        for (const likeElement of likeElements) {
            likeElement.addEventListener("click", () => {
                let currentPage = 'USER_POSTS_PAGE';
                // Проверяем стоит ли уже лайк
                if (likeElement.dataset.isliked === 'true') {
                    console.log(currentPage);
                    return dislike({ id: likeElement.dataset.postId, token: getToken(), currentPage, Id: likeElement.dataset.userId})
                }
                else {
                    console.log(currentPage);
                    return addLike({ id: likeElement.dataset.postId, token: getToken(), currentPage, Id: likeElement.dataset.userId });
